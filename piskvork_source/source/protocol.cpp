...
void vwrMessage(char *fmt, va_list va)
{
  ...
  if(!turNplayers || !turLogMsg) return;
  getMsgFn(fn);
  f=fopen(fn,"ab");
  if(f){
    vfprintf(f,fmt,va);
...
bool cmpExt(char *s,char *e)
{
  ...
  return len>0 && s[len]=='.' && stricmp(s+len+1,e)==0;
}
...
int Tplayer::move()
{
  ...
  if(!strnicmp(cutPath(brain2),"pbrain-",7)){
    if(mustSendBoard && moves<=DlastBoard+1 && moves>1){
      ...
      for(i=DlastBoard-1; i>=moves-1; i--){
        ...
        if(stricmp(buf,"OK")) goto l1; //not supported
      }
      mustSendBoard=false;
    }
    if(mustSendBoard){
l1:
      ...
      if(stricmp(buf,"OK")){
        ...
        if((width!=20 || height!=20) && !turNplayers &&
          (!strnicmp(buf,"UNKNOWN",7) && width!=height || 
          !strnicmp(buf,"ERROR",5) && strstr(buf,"20"))){
          softPause();
