...
TfileName fnrec,fnskin,fnbrain,fnstate,TahDat,PlochaDat,TimeOutsDat,MsgDat,InfoDat;
...
void savePsq()
{
  ...
  for(;;){
    ...
    if(GetSaveFileName(&psqOfn)){
      savePsq(psqOfn.lpstrFile, psqOfn.nFilterIndex);
      break; //ok
    }
    if(CommDlgExtendedError()!=FNERR_INVALIDFILENAME
      || !psqOfn.lpstrFile[0]) break; //cancel
  ...
  case WM_COMMAND:
   ...
   switch(wP){
    ...
    case IDOK:
     ...
     turRule= getRadioButton(hWnd,570,571);
 ...
 case WM_LBUTTONDBLCLK:
   ...
   for(i=0; i<2; i++){
    if(onAIName(i,lP)){
      ...
      if(GetOpenFileName(&brainOfn)){
        Tplayer *p= &players[i];
        if(stricmp(p->brain,brainOfn.lpstrFile) || !p->isComp){
          killBrains();
...
int pascal WinMain(HINSTANCE hInstance,HINSTANCE hPrevInst,LPSTR cmdLine,int cmdShow)
{
 ...
 initOpeningTab();
 if(__argc==4){
   if(!strcmp(__argv[1],"-p")){
     cmdLineGame=true;
     strncpy(players[0].brain,__argv[2],sizeof(players[0].brain)-1);
     strncpy(players[1].brain,__argv[3],sizeof(players[0].brain)-1);
 ...
 while(GetMessage(&msg, NULL, 0, 0)==TRUE)
  if(msg.hwnd==msgWnd || !TranslateAccelerator(hWin,haccel,&msg)){
    if(!logDlg || !IsDialogMessage(logDlg,&msg))
    if(!msgDlg || !IsDialogMessage(msgDlg,&msg))
    if(!resultDlg || !IsDialogMessage(resultDlg,&msg)){
