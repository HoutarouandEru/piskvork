...
void savePsq(char *fn, int format)
{
  Psquare p;
  FILE *f;
  int h,d;
...
start:
  ...
  if(!f){
    ...
    if(format!=2 && turNplayers) wrNames(f,0);
...
void turResult()
{
  ...
  *s++='\n';
  ...
  s+=sprintf(s,lng(600,"Time for turn: %d %s,   Time for match: %s\r\nTolerance: %d %s,   Memory: %d MB\r\nOpenings: %s\r\nGames played: %d"),
    ...
    maxMemory, buf1, turGamesCounter);
...
void init()
{
  ...
  //alocate board
  aminmax(height,10,80);
  aminmax(width,10,120);
  delete[] board;
  height2=height+2;
...
void newGame(int pl, bool openingEnabled)
{
  int j,x,y,x0,y0,w,r;
  signed char *o;
  ...
  if(autoBegin && openingEnabled){
    x0= width/2+rnd(4)-2;
    y0= height/2+rnd(4)-2;
    r= rnd(8);
    doMove1(Square(x0,y0));
...
bool doMove1(Psquare p, int action)
{
  ...
  for(i=0;i<4;i++){
    ...
    if((exactFive && !turNplayers) ? poc==5 : poc>=5){
      nxtP(p1,1);
      prvP(p2,1);
      //win
      ...
      printWinLine(p);
      printScore();
      if(!continuous || turNplayers) finishGame();
      break;
    }
  }
